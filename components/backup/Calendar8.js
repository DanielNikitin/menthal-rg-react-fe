    ...
    const fetchData = async () => {
      try {
        if (!dataFetched) {
          ...
          // Установка данных о доступном времени в состояние
          setAvailableTimesForMonth(resAvailableTimes.data.AvailableTimesForMonth);


          //setCurrentMonthData(resMonthData.data);  // (resMonthData.data.currentMonthData); конкретное указание раздела
          //console.log(resMonthData.data.currentMonthData.currentNumberOfDays);  // пример запроса для получения currentNumberOfDays
...
const handleDateClick = (day, month, year) => {
  month = month < 10 ? `0${month}` : month; // Добавляем ведущий ноль к месяцу, если он однозначный
  day = day < 10 ? `0${day}` : day; // Добавляем ведущий ноль к дню, если он однозначный
  const selectedDate = `${year}-${month}-${day}`; // Форматируем дату

  // Ищем выбранную дату в массиве availableTimesForMonth
  const selectedDateData = availableTimesForMonth.find(dateData => dateData.date === selectedDate);

  // Проверяем, найдена ли выбранная дата
  if (selectedDateData) {
    const availableTimes = selectedDateData.availableTimes;

    // Проверяем доступное время
    if (availableTimes.length > 0) {
      console.log(`Доступное время для ${selectedDate}: ${availableTimes}`);
    } else {
      console.log(`На ${selectedDate} свободное время отсутствует`);
    }
  } else {
    console.log(`Данные для ${selectedDate} отсутствуют`);
  }
};
